...
public class MyWorld extends World
{
    ...
    public void playerBlackStrategy(){//qwerty
        ...
        int colToPlace = 1;
        if(winPlays.size() > 0){
            colToPlace = winPlays.get(0) + 1;
        }else if(loosePlays.size() > 0){
            colToPlace = loosePlays.get(0) + 1;
        }else{
            colToPlace = ((int)(Math.random()*7)) + 1;
            colToPlace = clearColumns.get(((int)(Math.random()*clearColumns.size()))) + 1;
        }

        placeCol(colToPlace-1,colorInitial,image1);  //plays in the specified col and fills the 2D array with the 2nd parameter String
    }
